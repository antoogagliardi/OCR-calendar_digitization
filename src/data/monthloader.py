...
class MonthDataModule(pl.LightningDataModule):
    def __init__(self, encode_dict:dict, dataset:dict[Dataset], test_set:Dataset, saved_db_folder:str,
        ...
        super(MonthDataModule, self).__init__()
        ...
        if reload_data == True:
            if f'months_trainDataset.db' in os.listdir(saved_db_folder) and f'months_validDataset.db' in os.listdir(saved_db_folder):
                print('-- Loading existing dataset --')
                training_data = torch.load(os.path.join(saved_db_folder, f'months_trainDataset.db'), weights_only=False)
                validation_data = torch.load(os.path.join(saved_db_folder, f'months_validDataset.db'), weights_only=False)
            ...
            if f'months_testDataset.db' in os.listdir(saved_db_folder):
                self.test_dataset = torch.load(os.path.join(saved_db_folder, f"month_testDataset.db"), weights_only=False)
            ...
            if dataset != None:
                ...
                torch.save({"data": self.train_dataset}, os.path.join(saved_db_folder, f'months_trainDataset.db'))
                torch.save({"data": self.valid_dataset}, os.path.join(saved_db_folder, f'months_validDataset.db'))
            ...
            if test_set != None:
                self.test_dataset = test_set; torch.save(self.test_dataset, os.path.join(saved_db_folder, f"months_testDataset.db"))
